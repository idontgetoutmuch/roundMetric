/-
# The spherical chart on S² and its coordinate (co)frame

This file contains only what is needed to state and prove that the round metric on
`S²` equals `dθ² + sin²θ dφ²` in the spherical chart: the sphere, the chart
`sphericalChart` (`sphInv`/`sphFwd` on `sphSource`), the coordinate functions
`θ_coord`/`φ_coord` and their smoothness, the coordinate coframe `{dθ, dφ}`, the dual
frame `{Xθ, Xφ}`, and the duality `frame_dual`.

It is trimmed from the fuller `MercatorGeom`: the bundled-section differentiability
lemma, the orthonormal compass frame, and the connection (`covDerivOfFrame` /
`mercatorCov`) are omitted, as none is needed for the metric result.
-/
...
lemma sph_open_source : IsOpen sphSource := by
  ...
  simp only [Fin.isValue, ne_eq, preimage_ofPred_eq]
...
lemma θ_coord_contMDiffAt {x : S2} (hx : x ∈ S2_open) :
  ...
  have : Fact (Module.finrank ℝ (EuclideanSpace ℝ (Fin 3)) = 2 + 1) :=
    ⟨by simp only [finrank_euclideanSpace, Fintype.card_fin]⟩
  have hz := z_sq_lt_one hx
  have hcoe : ContMDiffAt (𝓡 2) 𝓘(ℝ, EuclideanSpace ℝ (Fin 3)) ⊤
      (fun p : S2 => (p : EuclideanSpace ℝ (Fin 3))) x :=
    (contMDiff_coe_sphere (n := 2)).contMDiffAt
  have h2 : ContMDiffAt (𝓡 2) 𝓘(ℝ, ℝ) ⊤ (fun p : S2 => (p.val 2 : ℝ)) x :=
    ((ContinuousLinearMap.contMDiff (EuclideanSpace.proj (2 : Fin 3))).contMDiffAt).comp x hcoe
  have h_arccos : ContDiffAt ℝ ⊤ Real.arccos (x.val 2) :=
    Real.contDiffAt_arccos (by nlinarith) (by nlinarith)
  have hcomp : ContMDiffAt (𝓡 2) 𝓘(ℝ, ℝ) ⊤
      (Real.arccos ∘ fun p : S2 => (p.val 2 : ℝ)) x :=
    h_arccos.contMDiffAt.comp x h2
  refine hcomp.congr_of_eventuallyEq (Filter.Eventually.of_forall fun p => ?_)
...
lemma φ_coord_contMDiffAt {x : S2} (hx : x ∈ sphSource) :
  ...
  have hmem : ((x.val 0 : ℂ) + (x.val 1 : ℂ) * Complex.I) ∈ Complex.slitPlane := by
    ...
    simpa only [sphSource, Set.mem_setOf_eq, Fin.isValue, Complex.add_re,
      Complex.ofReal_re, Complex.mul_re, Complex.I_re, mul_zero, Complex.ofReal_im,
      Complex.I_im, mul_one, sub_self, add_zero, Complex.add_im, Complex.mul_im,
      zero_add, ne_eq] using hx
  ...
  have hfun : φ_coord = fun p : S2 => Complex.arg ((p.val 0 : ℂ) + (p.val 1 : ℂ) * Complex.I) := by
    funext p
    show Complex.arg (Complex.equivRealProd.symm (p.val 0, p.val 1)) = _
    norm_num [Complex.equivRealProd_symm_apply]
  rw [hfun]; exact h_arg
...
lemma sphInv_contMDiff : ContMDiff 𝓘(ℝ, EuclideanSpace ℝ (Fin 2)) (𝓡 2) ⊤ sphInv := by
  have : Fact (Module.finrank ℝ (EuclideanSpace ℝ (Fin 3)) = 2 + 1) :=
    ⟨by simp only [finrank_euclideanSpace, Fintype.card_fin]⟩
  have hcomp : ContDiff ℝ ⊤ (fun q : EuclideanSpace ℝ (Fin 2) =>
      (![Real.sin (q 0) * Real.cos (q 1),
         Real.sin (q 0) * Real.sin (q 1),
         Real.cos (q 0)] : Fin 3 → ℝ)) := by
    refine contDiff_pi.2 fun i => ?_
    fin_cases i <;>
      simp only [Fin.zero_eta, Fin.mk_one, Fin.reduceFinMk, Fin.isValue,
        Matrix.cons_val_zero, Matrix.cons_val_one, Matrix.cons_val] <;>
      fun_prop
  have hvec : ContDiff ℝ ⊤ sphInvVec :=
    ((EuclideanSpace.equiv (Fin 3) ℝ).symm.contDiff).comp hcomp
  -- `Set.codRestrict sphInvVec (sphere 0 1) sphInvVec_mem`.
  exact ContMDiff.codRestrict_sphere (contMDiff_iff_contDiff.mpr hvec) sphInvVec_mem
...
lemma sphFwd_mdiffAt {x : S2} (hx : x ∈ sphSource) :
  ...
  · have h_diff : MDifferentiableAt (𝓡 2) (𝓘(ℝ, Fin 2 → ℝ)) (fun p => ![θ_coord p, φ_coord p]) x := by
      ...
      · rw [ mdifferentiableAt_iff ] at h_diff;
        ...
        · intro x_1 _
          simp only [Nat.succ_eq_add_one, Nat.reduceAdd, writtenInExtChartAt, extChartAt, OpenPartialHomeomorph.extend,
    modelWithCornersSelf_partialEquiv, PartialEquiv.trans_refl, PartialHomeomorph.toFun_eq_coe,
    OpenPartialHomeomorph.coe_toPartialHomeomorph, PartialHomeomorph.coe_toPartialEquiv_symm,
    OpenPartialHomeomorph.coe_toPartialHomeomorph_symm, Function.comp_apply]
          ext i; fin_cases i <;> rfl
        · simp only [Nat.succ_eq_add_one, Nat.reduceAdd, writtenInExtChartAt, extChartAt, OpenPartialHomeomorph.extend,
    modelWithCornersSelf_partialEquiv, PartialEquiv.trans_refl, PartialHomeomorph.toFun_eq_coe,
    OpenPartialHomeomorph.coe_toPartialHomeomorph, PartialHomeomorph.coe_toPartialEquiv_symm,
    OpenPartialHomeomorph.coe_toPartialHomeomorph_symm, Function.comp_apply, mem_chart_source,
...
lemma frame_dual {x : S2} (hx : x ∈ sphSource) (v : TangentSpace (𝓡 2) x) :
  ...
  rw [ ← Dinv.map_add ] ; congr ; ext i ; fin_cases i <;> simp +decide only [Fin.isValue, Fin.zero_eta, PiLp.add_apply, PiLp.smul_apply, EuclideanSpace.single_apply, ↓reduceIte, smul_eq_mul, mul_one, mul_zero, add_zero, Fin.mk_one, zero_add]
